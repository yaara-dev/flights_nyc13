...
The merged dataset containes 47 variables and 276688 observations.
...
In order to optimize our model we decreased number of categories (levels) in factorial variables which are identified with large number of categories. This step was performed using the statistical permutation test on each variable separately. Our assumption is that in a particular variable, a category which belonged to a delayed flights not by chance should be considered individually in our model since it has the potential to indicate on the possibility of delay of future flight related to this category. However, a category that repeats on random realizations with the same or even larger prevalence of delayed flights will probably fail to reject the null hypothesis, that delayed flights are absolutely not over-represented in the current category in the origin dataset. The insignificant categories of a variable were combined together into one aggregated category.

We performed 2000 realizations of the original dataset, During each realization we randomly shuffled departure time delay ('dep_delay') column, we counted how many flights identified as delayed (with dep_delay=1) for each category in each variable separately. Then, we compared it to the original count of departure delayed flights in the particular category. P-value for a given category was calculated as the number of realizations when the count of departure delayed flights was equal or even higher than the original delayed flights count in this category (plus one), divided by the total number of realizations. For example, a permutation analysis p-value of 1e-3 means that the null hypothesis was rejected in all the realizations, suggesting that the specific category significant in distinguishing(?) delayed flights in the original dataset, using confidence of 0.005. Finally, the permutation analysis p-value was corrected for multiple testing using the Benjamini-Hochberg procedure.


We applied this step on several variables:
  * "model" variable which included at first 127 categories, and after permutation test had only 13 categories.
* "manufacturer" variable which had initially 35 categories and finally had 4.
* "manu_model" included 147 categories at the beginning, and 12 at the end of permutation test.
* "seats" included 48 categories at the beginning, and 8 at the end of permutation test.
* "dest" variable which included at first 100 categories, and after permutation test had only 52 categories.

We provide in this project the complete output data frames observed from the permutation analysis. There are 5 data frames - each variable as we mentioned above has its own data frame with number of realizations (plus one) in which delayed flights count was equal or higher than the original count, for each category. We visualized the results for each variable in bar plots representing p-value adjusted values (after Benjamini-Hochberg correction) of each category. A line representing FDR=0.05 is presented on the plot as well in order to emphasize cases (categories) when the null hypothesis was rejected. Also, we generated plots including the total flights in each category to verify whether the results related to the amount in each category.
...
Following permutation test and its visualization, in each variable column (5 variables) we replaced the original levels (categories) to new levels. Those who passed the Benjamini-Hochberg multiple test correction (FDR<0.05) were kept, while the insignificant categories converted to single new category named 'other'.
Categories in 'model', 'manufacturer' and, 'manu_model' variables were changed to one-letter levels, as well as 'dest' variable whose categories were replaced to numbers for visualization improvement purposes.
...
Run classification desicion tree on train set
