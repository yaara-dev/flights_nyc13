...
Merge datsets with information about flights, planes, weather and airports
```{r}
...
In order to optimize our model we decreased the number of categories (levels) in factorial variables which are identified with large number of categories. This step was performed using the statistical permutation tests on each variable separately. Our assumption is that in a particular variable, a category which belonged to a delayed flights not by chance should be considered individually in our model since it has the potential to indicate on the possibility of delay of future flight related to this category. however, a category that repeats on random iterations with the same or even larger prevalence of delayed flights would be identified with large p-value, and thus considered as not significant category of delayed vs. not delayed flights. The insignificant categories of a variable were combined together into one aggregated category. We applied this step on several variables: 
...
train_test_split <- function(balanced_data) {
  sample_train <-
    sample(c(TRUE, FALSE),
           ...
           replace = TRUE,
           prob = c(0.8, 0.2))
  train <- balanced_data[sample_train,]
  test <- balanced_data[!sample_train,]
