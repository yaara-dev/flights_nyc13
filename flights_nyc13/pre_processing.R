...
#remove irrelevant columns
flights_full<-select(flights_full, -c(time_hour, arr_delay, flight, tailnum, arr_time, dep_time, speed, wind_gust))
...
split_ind<-split(ind_vec, cut(seq_along(ind_vec), num_categories_dep_delay, labels = FALSE))
for (i in 1:length(split_ind)){
  flights_full_arranged$dep_delay[unlist(split_ind[i])]<-as.numeric(names(split_ind[i]))
}

#plot dep_delay categories counts
ggplot(flights_full_arranged, aes(dep_delay, fill=dep_delay))+ geom_bar(fill=c('#660000','#993333', '#CC6666', "#FFCCCC")) + 
...
manu_model<-paste(flights_full_arranged$manufacturer, flights_full_arranged$model, sep="_")
flights_full_arranged$manu_model<-manu_model

levels_manu_model<-levels(factor(flights_full_arranged$manu_model))

#bar plot of manufacturer_model = "AGUSTA SPA_A109E" for example
manu_model_df<-flights_full_arranged%>% filter(manu_model==levels_manu_model[1])%>%group_by(dep_delay)%>% summarise(total_counts=n())
manu_model_df$normalized_counts<-manu_model_df$total_counts/sum(manu_model_df$total_counts)
threshold_uniform_hist<-sum(manu_model_df$normalized_counts)/num_categories_dep_delay
ggplot(manu_model_df, aes(x = dep_delay, y=normalized_counts)) + 
  geom_bar(stat="identity")+ labs(x="Departure delay categories", y="normalized counts of flights",title=paste('manu_model =',levels_manu_model[1])) +theme(plot.title = element_text(hjust = 0.5, size=19, face="bold")) +
  geom_hline(yintercept=threshold_uniform_hist, linetype="dashed", color = "red", size=1.5)
SSE<-sum((manu_model_df$normalized_counts-threshold_uniform_hist)^2)
SSE
SSE_vec_manu_model<-sapply(levels_manu_model, function(a_model){
  manu_model_df<-flights_full_arranged%>% filter(manu_model==a_model)%>%group_by(dep_delay)%>% summarise(total_counts=n())
  manu_model_df$normalized_counts<-manu_model_df$total_counts/sum(manu_model_df$total_counts)
  threshold_uniform_hist<-sum(manu_model_df$normalized_counts)/num_categories_dep_delay
  SSE<-sum((manu_model_df$normalized_counts-threshold_uniform_hist)^2)
  SSE
...
manu_model_SSE_df<-data.frame(manu_model=names(SSE_vec_manu_model), SSE=SSE_vec_manu_model) 
rownames(manu_model_SSE_df)<-1:length(manu_model_SSE_df$manu_model)
flights_counts_manu_model_df<- flights_full_arranged %>% group_by(manu_model)%>% summarise(total_counts=n()) 
manu_model_SSE_df<-merge(manu_model_SSE_df, flights_counts_manu_model_df, by="manu_model") #add flights counts per manu_model to manu_model_SSE_df 
manu_model_SSE_df<- manu_model_SSE_df%>% arrange(SSE_vec_manu_model) #arrange df in ascending order by SSE value per manu_model
# barplot SSE value per manufacturer
ggplot(manu_model_SSE_df, aes(x = reorder(manu_model, SSE) , y=SSE)) + geom_bar(stat="identity") + labs(title="SSE value per manu_model", x="manu_model", y="SSE value") +
  theme(plot.title = element_text(hjust = 0.5, size=19, face="bold"), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=6))
ggplot(manu_model_SSE_df, aes(x = reorder(manu_model, SSE) , y=total_counts)) + geom_bar(stat="identity") + labs(title="flights counts per manu_model", x="manu_model", y="flights counts") +
  theme(plot.title = element_text(hjust = 0.5, size=19, face="bold"), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=6))

num_categories_manu_model<-4
ind_vec_manu_model<-as.numeric(rownames(manu_model_SSE_df))
split_ind_manu_model<-split(ind_vec_manu_model, cut(seq_along(ind_vec_manu_model), num_categories_manu_model, labels = FALSE))
manu_model_SSE_df$new_category<-rep(0,length(manu_model_SSE_df$manu_model))
for (i in 1:length(split_ind_manu_model)){
  manu_model_SSE_df$new_category[unlist(split_ind_manu_model[i])]<-as.numeric(names(split_ind_manu_model[i]))
}
# barplot SSE value per model with categories thresholds
ggplot(manu_model_SSE_df, aes(x = reorder(manu_model, SSE) , y=SSE)) + geom_bar(stat="identity") + labs(title="SSE value per manu_model", x="manu_model", y="SSE value") +
  theme(plot.title = element_text(hjust = 0.5, size=19, face="bold"), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=6)) +
  geom_vline(xintercept=c(split_ind_manu_model$`2`[1], split_ind_manu_model$`3`[1], split_ind_manu_model$`4`[1]), linetype="dashed", color = "red", size=1.3)
...
for (j in 1:length(manu_model_SSE_df$manu_model)){
  flights_full_arranged$manu_model<-replace(flights_full_arranged$manu_model, flights_full_arranged$manu_model==manu_model_SSE_df$manu_model[j], manu_model_SSE_df$new_category[j])
}
flights_full_arranged$manu_model<-as.factor(flights_full_arranged$manu_model)

#plot number of flights per manu_model category
df_manu_model_counts<-flights_full_arranged %>% group_by(manu_model) %>% summarise(total_counts=n())
ggplot(df_manu_model_counts, aes(x = manu_model, y=total_counts, fill=manu_model)) + geom_bar(stat="identity") + labs(title="Flights counts per manu_model category", x="manu_model category", y="Total counts of flights") +
  theme(plot.title = element_text(hjust = 0.5, size=19, face="bold")) + scale_fill_manual(values=c("#CC99FF",'#9966CC','#663399','#330066'))


# remove model column and manufacturer column from flights_full_arranged
flights_full_arranged<-select(flights_full_arranged, -c(model, manufacturer))
