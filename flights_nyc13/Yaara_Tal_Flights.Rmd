...
Before running the prediction model, we would like to choose the best parameters for optimizing the model.
Thus, we used the machine learning 'mlr' R package that supplies tools to train several different models and to perform tuning for hyperparameters. Using this tools we can view each hyperparameters' impact on the performance of the model.
In rpart decision tree library, we control the parameters using the rpart.control() function.
...
Build the model according to best parameters 
