...
First, we plotted weather variables vs. departure delay in order to visually spot if there is a linear relationship between them. On top of the scatter plot there is a linear model fit line:
...
we would like to examine correlation between numeric variables. 
...
Thus, we perform Spearman correlation test between numerical variables in the dataset::
```{r}
...
We decided to exclude manu_model from model analysis since there is a correlation between manu_model and model manufacturer - show test/statistical proves!
```{r echo=FALSE}
