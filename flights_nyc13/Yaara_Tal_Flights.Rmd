...
In order to predict successfully a delayed flight we would like to use most affecting features (variables) in our model and avoid from features that may increase noise. Achieving this goal requires pre-processing steps in which we reduce the number of variables and the number of levels/categories in variables.

First, we plotted numeric variables vs. departure delay variable in order to visually spot if there is a linear relationship between them. On top of the scatter plot there is a linear model fit line:
...
dewp and temp are highly correlated (0.9), so are wind speed and wind gust (0.87), and air time and distance (0.98).The correlation is significant (pval\<0.05) We decided to remove dewp, air time, and wind gust to avoid double variables in the model. All three variables are an outcome in some way of the variable they are highly correlated to.
...
### Merge Manufacturer + Model Variables

We found that part of model planes' were repeated in several manufacturers. It means that when two planes share the same model name but different manufacturers, they are Not actually recognized by exactly the same model. As a result, we decided to combine 'manufacturer' and 'model' columns into one column named 'manu_model' with the aim of reducing the amount of variables in the dataset.
...
We have seen that overall there isn't one particular variable that has an obvious direct relationship to the departure delay. The purpose of the next plot is to examine the option of multiple variables together having a more clear relation to the departure delay.
...
As we prefer to reduce number of predicting variables in our model to prevent confusion we removed 'model' and 'manufacturer', and let the model basing on 'mau_model' variable.
...
We should balance the dataset because the amount of delays is \~20%
...
First, we would like to choose best parameters for optimizing model's prediction. Thus, we will use 'mlr' package 
...
Run classification decision tree on train set with best max depth and min split parameters' values. We chose cp=0.0004 instead of cp=0.0001 despite it outputs worse accuracy test, because cp=0.0004 enables visualization and interpretation of important predicting variables required in this project. 

```{r}
# build model according to best parameters 
model_best_param = rpart(dep_delay~ .,   data=train, method = 'class', control = c(cp=0.0004, maxdepth = dt_tuneparam$x$maxdepth, minsplit = dt_tuneparam$x$minsplit))
```
...
rpart.plot(model_best_param, split.fun = split.fun)
...
We would like to estimate the accuracy - a metric used in classification problems used to tell the percentage of accurate predictions. Accuracy is calculated by dividing the number of correct predictions by the total number of samples.

```{r}
predicted_values<- predict(model_best_param, test, type = 'class')
...
Earlier on we chose 20 minutes as a threshold to determine if a flight is considered delayed.We generated two additional datasets: one with a higher threshold (25 minutes) and one with a lower threshold (15 minutes) in order to test the sensitivity of our predictions. Let's run our model on these datasets: Let's run our model on these datasets:
...
In the first step, we ran a decision tree model. This model reached an accuracy level of about 70% in delay prediction. In the second step, we wanted to check whether a Random Forest model could learn the data better and give a more accurate prediction. We thought this model would be better because #add explanation. The Random Forest model reached a much higher level of accuracy than the decision tree, 98%. In principle, the chance of an overfit in Random Forest is low thanks to bagging and random feature selection, they are trained independently on different subsets of the training data. However, the high accuracy score raises our concern that the model may overfit the data. In order to examine the matter, we performed another run of the Random Forest model where in this run, we limited the depth of the trees and increased the number of trees (from 1000 to 2000). The result of this run showed 94% accuracy, which is still high but is very unlikely to be overfitted to the large number of trees and the limitation of the trees' depth. Further, model is highly accurate for the test as well, so on the one hand, it may be just excellent at predicting. Another possible explanation is that there is indeed an overfit resulting from the fact that the training and the test are similar since they were taken from the same data set.

In conclusion, we have shown that it is possible to predict if there will be a delay in takeoff with a high probability using a random forest model that considers various parameters related to weather, aircraft characteristics, airport, destination, airlines, etc. For further research, it is worthwhile to implement the model on current databases for two reasons; The first is to verify the model and see that it is not adjusted only for 2013. Second, since the Corona epidemic, the world of flights has changed, i.e., the airlines have changed partly due to bankruptcies of low-cost companies, and the loads at the airport have changed as well.
