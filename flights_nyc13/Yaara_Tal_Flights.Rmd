...
This project aims to generate a prediction model of departure delay time. A reliable prediction will benefit airports and airlines, passengers, and global environmental sustainability. 
...
In order to optimize our model, we decreased the number of categories (levels) in factorial variables that are identified with a large number of categories. This step was performed using the statistical permutation test on each variable separately. Our assumption is that in a particular variable, a category that belonged to a delayed flight, not by chance, should be considered individually in our model since it can indicate the possibility of delay of a future flight related to this category.
However, a category that repeats on random realizations with the same or even larger prevalence of delayed flights will probably fail to reject the null hypothesis that delayed flights are not over-represented in the current category in the original dataset. The insignificant categories of a variable were combined into one aggregated category.
...
The P-value for a given category was calculated as the number of realizations when the count of departure delayed flights was equal to or even higher than the original delayed flight count in this category (plus one), divided by the total number of realizations.
For example, a permutation analysis p-value of 1e-3 means that the null hypothesis was rejected in all the realizations, suggesting that the specific category is significant in distinguishing delayed flights in the original dataset, using a confidence of 0.005. Finally, the permutation analysis p-value was corrected for multiple testing using the Benjamini-Hochberg procedure.
...
We provide in this project the complete output data frames observed from the permutation analysis. There are 5 data frames - each variable has its data frame with the number of realizations (plus one) in which delayed flights count was equal to or higher than the original count for each category.
We visualized the results for each variable in bar plots representing p-value adjusted values (after Benjamini-Hochberg correction) of each category. A line representing FDR=0.05 is presented on the plot as well in order to emphasize cases (categories) when the null hypothesis was rejected. In addition, we generated plots of total flights in each category to verify whether the results related to the amount in each category
```{r}
