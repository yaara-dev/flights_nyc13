...
Before running prediction model, we would like to choose best parameters for optimizing the model. Thus, we used the machine learning 'mlr' R package that supplies tools to train several different models and perform tuning for hyperparameters. Using this tools we can view each hyperparameter impact on the performance of the model.
...
confusion_class_15 <- confusionMatrix(confusion_class_15)
confusion_class_15
...
confusion_class_25 <- confusionMatrix(confusion_class_25)
confusion_class_25
...
Running random forest model followed with sensitivity checks. We applied the current random forest model on a data based on 20 minutes threshold as definition for delayed flight and 15 minutes threshold, as we performed in simple classification decision tree.
